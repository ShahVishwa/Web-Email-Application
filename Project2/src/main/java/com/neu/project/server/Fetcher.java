		...
		try {
			if (connection != null) {
				...
				while (rs.next()) {
					...
					ed.userName = rs.getString(2);
					ed.password = rs.getString(3);
...
//		Folder starredFolder = null;
		try {
			...
			store.connect("imap.gmail.com", emaildata.userName,
					emaildata.password);
...
//			starredFolder = store.getFolder("Starred");
			...
			for (Folder f : folders) {
				try {
					...
					if(folderName.contains("inbox")||folderName.contains("trash")||folderName.contains("sent")||folderName.contains("draft"))
...
//			}
	...
	private void saveMessage(EMailData emaildata, String subject,
		...
		String emailDate = sdf.format(sendDate);
		...
		try {
			if (connection != null) {
				...
				stmt.setString(2, emailDate);
				stmt.setInt(3, emaildata.userId);
